 ...
 */
class doc_generic_don_odt extends ModeleDon
{
	/**
	 * Issuer
	...
	public function __construct($db)
	{
		...
		$this->scandir = 'DON_ADDON_PDF_ODT_PATH'; // Name of constant that is used to save list of directories to scan
	...
	public function info($langs)
	{
		...
		$texte .= '<input type="hidden" name="param1" value="DON_ADDON_PDF_ODT_PATH">';
		...
		$texte .= $conf->global->DON_ADDON_PDF_ODT_PATH;
		$texte .= '</textarea>';
		...
		if (!empty($conf->global->DON_ADDON_PDF_ODT_PATH)) {
		...
		if ($nbofiles) {
			...
			foreach ($listoffiles as $file) {
				$texte .= '- '.$file['name'].' <a href="'.DOL_URL_ROOT.'/document.php?modulepart=doctemplates&file=dons/'.urlencode(basename($file['name'])).'">'.img_picto('', 'listlight').'</a>';
				$texte .= ' &nbsp; <a class="reposition" href="'.$_SERVER["PHP_SELF"].'?modulepart=doctemplates&keyforuploaddir=DON_ADDON_PDF_ODT_PATH&action=deletefile&token='.newToken().'&file='.urlencode(basename($file['name'])).'">'.img_picto('', 'delete').'</a>';
				$texte .= '<br>';
			}
			$texte .= '</div>';
		}
		...
		$texte .= '<input type="hidden" value="DON_ADDON_PDF_ODT_PATH" name="keyforuploaddir">';
	 ...
	 *
	 *	@param		Don		$object				Object source to build document
